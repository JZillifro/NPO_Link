...
class SearchBar extends Component {
  ...
  onSearchChange = (search_key, query) => {
     this.setState({
       search_key: search_key,
       query: query
     }, () => {
         this.props.onSearchChange(this.state.search_key, this.state.query);
     })
  }
  ...
  handleInputChange = () => {
    this.setState({
      ...
      if (this.state.query && this.state.query.length > 0) {
         ...
         this.props.onSearchChange(this.state.search_key, '');
      }
    })
  }

  onSearchKeyChange = (search_key) => {
     this.setState({
       search_key: search_key
     }, () => {
         this.props.onSearchChange(this.state.search_key, this.state.query);
    ...
    return (
      <Container className="mb-5 justify-content-center">
         <Row className="mb-5">
            ...
            <Col xs={1}>
               <DropdownChoices onClick={this.onSearchKeyChange}
                                items={this.props.search_keys}
                                value={this.props.initialSearchValue}>
               </DropdownChoices>
            </Col>
            <Col xs={3}>
